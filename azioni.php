...
if (isset($_GET['name'])  && isset($_GET['squadra'])) {
    ...
    if ($name == "modificasquadra") {
        if (isset($_POST['submit_modifica'])) {
            ...
            $sql_update = "UPDATE squadre SET nome=?, forza=?, colore1=?, colore2=? WHERE utente=? AND nome=?";
            $stmt_update = $conn->prepare($sql_update);
            $stmt_update->bind_param("sissss", $nuovo_nome, $forza, $colore_primario, $colore_secondario, $user, $squadra);
